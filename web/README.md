...
The v0.2.2 release does not provide browser-native account onboarding or a
backend settings form. Register/login and external backend configuration use the
authenticated JSON APIs described in [cmd/server/README.md](../cmd/server/README.md)
and [caldav-connection-config.md](../docs/caldav-connection-config.md) until
[#145](https://github.com/chrisbelyea/momentum/issues/145) and
[#146](https://github.com/chrisbelyea/momentum/issues/146) are complete.

The authenticated board/list workflow is implemented. Remaining limitations and
future improvements are:

1. **Browser certification**: The full browser-level board/list, rich-field, failure-recovery, and concurrency workflow is covered by merged PR [#124](https://github.com/chrisbelyea/momentum/pull/124) and its green Chromium CI job. This does not certify account onboarding or backend settings pages.
...
### Running the Server
```bash
# Initialize a disposable development database with test data
./scripts/db/init_dev.sh

# Build and run the server
go build -o server ./cmd/server
DB_PATH=momentum_dev.db PORT=8443 MOMENTUM_DEV_MODE=1 ./server
```
